...
def get_units_to_data_share(db: Database, units: list):
    query = text('''
                    SELECT u.UnitNr, sa.UnitLicenceNr, sa.VIN_Number, u.CustomerCombiNr,branch.Region as Region_Name,
					branch.BranchName Owning_Branch_Name, cust.CustomerName Customer_Name,
                    branch.Country Country_Name,
					u.IntchType, u.IntchOutDateTime, u.CustomerNr, u.RateNr, u.MasterLeaseNr, 
                    ...
                    LEFT JOIN (SELECT Organization_Id,
							...
							ON fa_org.Root_Organization_Id = p_fa_org.Organization_Id and fa_org.Active = 1 and p_fa_org.Active=1
                                AND p_fa_org.FleetConnected_Ind = 'Y' and p_fa_org.Fleetradar_Ind = 'Y'
                    LEFT JOIN (SELECT Asset_Id, Unit_Nr, Unit_Licence_Nr UnitLicenceNr, VIN_Number, Active, Device_Pairing_Status,Fleet_Id 
    ...
    for curr_index in range(0, len(units), batch_size): 
        curr_unit_nrs = units[curr_index:curr_index + batch_size]
        params={"units": curr_unit_nrs, "insight_add_chrg_types":AdditionalChargeType.Insight_Additional_Charge_Types}
        result_set = db.query(statement=query, params=params, as_dataframe=True, params_to_expand=['units', 'insight_add_chrg_types'])
