...
from app.common.helpers.common_data_access import get_fa_organization_details, \
...
from app.asset_groups.deactivate_asset_group.deactivate_asset_group_data_access import get_cust_combi_numbers

deactivate_asset_group_bp = func.Blueprint()

@deactivate_asset_group_bp.function_name(name="Deactivate_Asset_Group")
@deactivate_asset_group_bp.route(route="deactivate/assetgroup",  methods=[func.HttpMethod.POST])
@global_exception_handler
def deactivate_asset_group_api(req: func.HttpRequest) -> func.HttpResponse:
    logger = logging.getLogger("Deactivate_Asset_Group")
    db = Database()
    ...
    if fa_org_details_dict['Root_Organization_Id'] is None:
        message = f"The FA organization (ID:{fa_org_id}) is a root organization hence cannot be deactivated."
        logger.warning(message)
        response = Response(status=True, message=message).getJsonResponse()
        return func.HttpResponse(
                response,
    ...
    if fa_org_details_dict['Fleetconnected_Ind'] in ('Y', 'y'):
        ...
        if not combi_number_removal_allowed:
            raise ScalarException(message=f"One or more combinumbers of FA organization {fa_org_id} has Insight active units. Hence, organization can't be deactivated.", 
        ...
        if response.status_code == 200:
                ...
                if len(all_data_json["subGroups"]) > 0:
                    raise ScalarException(message=f"Can not deactivate asset group {asset_group_id} with subgroup(s) in it", response_code=ResponseCode.INTERNAL_ERROR)
        ...
        update_fleetconnected_ind_in_fa_organization(db=db, fa_org_id=fa_org_id, has_fc_access=False)
        message = f"Asset group {asset_group_id} deactivated for the FA Organization {fa_org_id} in scalar"
        logger.info(message)
        response_code = ResponseCode.SUCCESS
        status = True
    else:
        message = "Cannot deactivate Asset Group for non-scalar (or) already deactivated FA organizations!"
        response_code = ResponseCode.BAD_REQUEST
        status = False
        
    response = Response(status=status, message=message).getJsonResponse()
    return func.HttpResponse(
            response,
